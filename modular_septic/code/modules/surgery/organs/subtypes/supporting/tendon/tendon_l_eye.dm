/obj/item/organ/tendon/l_eye
	name = "left medial rectus"
	zone = BODY_ZONE_PRECISE_L_EYE

/obj/item/organ/tendon/l_eye/robot
	name = "cyborg left medial rectus"
	icon_state = "tendon-c"
	base_icon_state = "tendon-c"
	status = ORGAN_ROBOTIC
	organ_flags = ORGAN_SYNTHETIC|ORGAN_LIMB_SUPPORTER|ORGAN_INDESTRUCTIBLE|ORGAN_NO_VIOLENT_DAMAGE

/obj/item/organ/tendon/l_eye/robot/get_wound_resistance(wounding_type = WOUND_BLUNT)
	if(wounding_type == WOUND_TENDON)
		return -5
