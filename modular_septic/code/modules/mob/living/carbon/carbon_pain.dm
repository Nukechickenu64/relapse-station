...
/mob/living/carbon/proc/endorphinate(silent = FALSE, no_endorphin_flash = FALSE, forced = FALSE)
	...
	if(!silent && !obvious_endorphination)
		playsound_local(src, endorphination_sound, 80, FALSE)
	else if(obvious_endorphination)
		playsound(src, special_endorphination_sound, 80, FALSE, 1) // It's meant to be a hearable sound
