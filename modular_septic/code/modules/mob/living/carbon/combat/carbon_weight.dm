...
/mob/living/carbon/proc/update_encumbrance_movespeed_modifier()
	switch(encumbrance)
		if(ENCUMBRANCE_EXTREME)
			add_or_update_variable_movespeed_modifier(/datum/movespeed_modifier/carry_weight, TRUE, get_basic_speed()*0.8)
		if(ENCUMBRANCE_HEAVY)
			add_or_update_variable_movespeed_modifier(/datum/movespeed_modifier/carry_weight, TRUE, get_basic_speed()*0.6)
		if(ENCUMBRANCE_MEDIUM)
			add_or_update_variable_movespeed_modifier(/datum/movespeed_modifier/carry_weight, TRUE, get_basic_speed()*0.4)
		if(ENCUMBRANCE_LIGHT)
			add_or_update_variable_movespeed_modifier(/datum/movespeed_modifier/carry_weight, TRUE, get_basic_speed()*0.2)
		if(ENCUMBRANCE_NONE)
			add_or_update_variable_movespeed_modifier(/datum/movespeed_modifier/carry_weight, TRUE, 0)
...
/mob/living/carbon/proc/encumbrance_text()
	switch(encumbrance)
		if(ENCUMBRANCE_EXTREME)
			return span_flashinguserdanger("EXTRA-HEAVY!!")
		if(ENCUMBRANCE_HEAVY)
