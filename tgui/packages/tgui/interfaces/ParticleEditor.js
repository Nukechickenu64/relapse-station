...
import { Box, Button, Collapsible, ColorBox, Flex, Input, LabeledList, NumberInput, Section, Tooltip, NoticeBox } from '../components';
...
const ParticleIntegerEntry = (props, context) => {
  ...
  return (
    <NumberInput
      ...
      stepPixelSize={5}
      width="39px"
      onDrag={(e, value) => act('modify_particles_value', {
        new_data: {
          name: name,
          value: value,
          type: 'int',
...
const ParticleTextEntry = (props, context) => {
  ...
  return (
    <Input
      value={value}
      width="250px"
      onInput={(e, value) => act('modify_particles_value', {
        new_data: {
          name: name,
          value: value,
          type: 'text',
...
const ParticleNumListEntry = (props, context) => {
  ...
  return (
    <Input
      value={valArr.join(',')}
      width="250px"
      onInput={(e, val) => act('modify_particles_value', {
        new_data: {
          name: name,
          value: val,
          type: 'numList',
...
export const ParticleEditor = (props, context) => {
  const { act, data } = useBackend(context);
  const particles = data.target_particles || {};
        ...
        <Section
          ...
          buttons={!hasParticles ? (
            <Button
              ...
              onClick={() => act('add_particles')} />
          ) : (<Button.Confirm
            icon="minus"
            content="Remove Particles"
            onClick={() => act("remove_particles")} />)} >
          <Collapsible title={"Debug Information"}>
            <NoticeBox danger> {String(Date.now())} <br />
              Particles? {hasParticles.toString()} -
              {(data.target_particles === null).toString()} <br />
              dazta Json - {JSON.stringify(data.target_particles)}
            </NoticeBox>
          </Collapsible>
          <GeneratorHelp />
          {!hasParticles ? (
